...
수천 건의 요청이 동시에 몰리는 선착순 이벤트에서 쿠폰이 중복 발급되고 재고 수량이 어긋났다.
**"락을 걸면 된다"에서 멈추지 않고, 락의 유효 범위를 저장소 단위로 넓혀가며 세 단계를 직접 구현**했다.

| 단계 | 방식 | 부딪힌 한계 |
|---|---|---|
| 1 | JVM 내 락 (`synchronized`, `ReentrantLock`) | 한 서버 안에서는 정합성 확보. 락 범위가 JVM 하나라 인스턴스를 늘리면 무효 |
| 2 | DB 비관적 락 | 인스턴스 간 정합성 확보. 잠금 대상이 DB 행에 한정돼, 잠가야 할 상태가 DB 밖(Redis 등)에 걸치면 임계 영역 전체를 감싸지 못함. 고부하 시 데드락·커넥션 고갈 위험도 확인 |
| 3 | Redis 분산 락 | 저장소와 무관하게 하나의 키로 임계 영역 전체를 잠글 수 있어 채택 |

**전환 후에도 중복 발급이 남았던 이유** — 락 설정이 아니라 트랜잭션 경계가 어긋나 있었다. 락 해제가
커밋보다 먼저 일어나 그 사이에 다른 스레드가 차감 전 재고를 읽었다. 커스텀 Transactional Advice로
트랜잭션 커밋 이후에 락을 반납하도록 순서를 바로잡아 해소했다.

**락 획득 방식 교체** — 대기 중인 스레드가 락 획득을 재시도하는 스핀락 방식이 Redis에 부하를 준다는 점을
확인하고, 해제 메시지를 받은 시점에만 재시도하는 Redisson의 pub-sub 기반 락으로 교체.

| 항목 | 결과 (2024년 당시 측정) |
|---|---|
| 1,000건 동시 요청 평균 응답시간 | **4,400ms → 1,800ms (약 59% 개선)** |

> 아래 실험 A·B는 **위 수치의 검증이 아니다.** 2024년 당시 조건(요청 수, 예열 여부)이 저장소에 남아
> 있지 않고 DB 구성도 다르다. 같은 코드베이스를 현재 조건에서 독립적으로 다시 측정한 기록이다.
